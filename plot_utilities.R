...
merge_draws_and_data <- function(
  dat,
  samples,
  categories = c("black", "other", "hisp", "white"),
  outcome_string = "births",
  denom_string = "pop",
  ...
  dat <- dat %>% mutate(exposure_code = exposed)

  dat <- dat %>%
    pivot_longer(
      cols = starts_with(paste0(outcome_string, "_")) |
        starts_with(paste0(denom_string, "_")),
  ...
  dat <- dat %>%
    ...
    mutate(
      category_code = as.numeric(factor(category, levels = categories))
    ) %>%
    mutate(time_code = as.numeric(factor(date))) %>%
    arrange(state, category, date)

  ban_group <- dat %>%
    filter(banned_state == 1) %>%
    group_by(date, start_date, end_date, category, category_code, time_code) %>%
    reframe(
      across(
        contains(outcome_string) | contains(denom_string),
        function(x) sum(x, na.rm = TRUE)
      ),
      exposure_code = ifelse(any(exposure_code == 1), 1, 0)
    ) %>%
    ungroup() %>%
    mutate(state = "States With Bans", banned_state = 1) %>%
    mutate(
      category_code = as.numeric(factor(category, levels = categories))
    ) %>%
    mutate(time_code = as.numeric(factor(date))) %>%
  ...
  unexposed_group <- dat %>%
    ...
    ungroup() %>%
    mutate(state = "States Without Bans", banned_state = 0) %>%
    mutate(
      category_code = as.numeric(factor(category, levels = categories))
    ) %>%
    mutate(time_code = as.numeric(factor(date))) %>%
  ...
  if (!any(categories == agg_category_name)) {
    dat_totals <- dat %>%
      ...
      mutate(
        category = agg_category_name,
        category_code = max(dat$category_code) + 1
      )

    dat <- bind_rows(dat, dat_totals)
  }
  ban_indices <- dat %>%
    filter(!(state %in% c("States With Bans", "States Without Bans"))) %>%
    select(banned_state, state) %>%
    distinct() %>%
    ungroup() %>%
    mutate(ban_index = row_number()) %>%
    filter(banned_state == TRUE) %>%
    pull(ban_index)
  ...
  unexposed_indices <- dat %>%
    ...
    ungroup() %>%
    mutate(ban_index = row_number()) %>%
    filter(banned_state == FALSE) %>%
    pull(ban_index)

  draws_mat <- samples %>% as_draws_matrix()

  if (verbose) {
    print("Getting ypred draws...")
  }
  start <- Sys.time()

  if (any(grep("^te", colnames(draws_mat)))) {
    merged_draws <- draws_mat %>%
      spread_draws(ypred[K, D, N], mu[K, D, N], te[K, D, N])
  ...
  if (verbose) {
    print("Merging data and draws...")
  }
  start <- Sys.time()
  merged_draws <- merged_draws %>%
    left_join(
      dat,
      by = c("N" = "time_code", "D" = "state_code", "K" = "category_code")
    ) %>%
    ungroup()
  if (verbose) {
    print(Sys.time() - start)
  ...
  merged_df_ban <- merged_draws |> filter(state == "States With Bans")
  merged_df_tx <- merged_draws |> filter(state == "TX")
  merged_df_ban_no_tx <- merged_df_ban

  merged_df_ban_no_tx <- merged_df_ban_no_tx |>
    mutate(
      ypred = merged_df_ban$ypred - merged_df_tx$ypred,
      deaths = merged_df_ban$deaths - merged_df_tx$deaths,
      births = merged_df_ban$births - merged_df_tx$births,
      mu = log(exp(merged_df_ban$mu) - exp(merged_df_tx$mu)),
      mu_treated = log(
        exp(merged_df_ban$mu_treated) - exp(merged_df_tx$mu_treated)
      ),
      state = "States With Bans (excluding Texas)",
      exposure_code = (merged_draws |>
        filter(state != "TX" & state != "States With Bans") |>
        group_by(.draw, date, category) |>
        summarize(exposure_code = max(exposure_code)))$exposure_code,
    )

  merged_draws <- bind_rows(merged_draws, merged_df_ban_no_tx)
  merged_draws
}

make_all_te_plots <- function(
  merged_df,
  quantiles_df,
  state_name = "Texas",
  category = "all",
...
make_state_fit_plot <- function(
  ...
  state_plot <- quantiles_df %>%
    ...
    ggplot() +
    geom_point(aes(x = date, y = .data[[target]])) +
    geom_ribbon(
      aes(x = date, ymin = ypred_lower, ymax = ypred_upper),
      alpha = 0.5
    ) +
    geom_line(aes(x = date, y = ypred_mean), color = "red") +
    theme_bw() +
    xlab("Date") +
    ggtitle(state_name)
  if (target == "births") {
  ...
  if ((quantiles_df$banned_state[quantiles_df$state == state_name])[1]) {
    quantiles_df %>%
      filter(state == !!state_name, exposure_code == 1) %>%
      summarize(treatment_date = first(date)) %>%
...
make_gap_plot <- function(
  ...
  quantiles_df <- quantiles_df %>%
    filter(category == !!category & state == !!state_name)

  quantiles_df %>%
    filter(state == !!state_name, exposure_code == 1) %>%
    summarize(treatment_date = first(date)) %>%
    pull(treatment_date) -> treatment_date
  if (is.na(treatment_date)) treatment_date <- max(quantiles_df$date)

  quantiles_df %>%
    mutate(pre_ban = date < treatment_date) %>%
    ggplot() +
    # geom_ribbon(
    #   aes(
    #     x = date,
    ...
    # ) +
    geom_line(
      aes(x = date, y = .data[[target]] / ypred_mean - 1, group = pre_ban),
      color = "red"
    ) +
    theme_bw() +
    geom_hline(
...
make_births_histogram <- function(
  merged_df,
  state_name = "Texas",
  category = "all",
  treatment_date = NULL,
...
make_violins <- function(
  ...
  merged_df <- merged_df %>%
    filter(
      !state %in%
        c("Ban States", "Ban States (excl. Texas)", "States Without Bans")
    )
  ...
  if (method == "pred") {
    ...
    if (target == "births") {
      ...
      state_df <- merged_df %>%
        group_by_at(c(".draw", ".chain", group_var, "date", "state")) %>%
        mutate(
          treated = sum(exp(mu_treated)),
          untreated = sum(exp(mu)),
          denom = mean(.data[[denom]] * years)
        ) %>%
        ungroup()

      ban_states_df <- state_df %>%
        group_by_at(c(".draw", ".chain", "category", "date")) %>%
        summarize(
          treated = sum(treated),
          untreated = sum(untreated),
          denom = sum(denom)
        ) %>%
        mutate(
          D = max(state_df$D) + 1,
      ...
      state_df <- bind_rows(
        state_df %>% select_at(colnames(ban_states_df)),
        ban_states_df,
        ban_states_no_tx_df
      )
  ...
  vp <- state_df %>%
    ...
    geom_hline(
      yintercept = ifelse(estimand == "diff", 0, 1),
      col = "red",
      linetype = "dashed"
    ) +
    geom_text(
      data = stats_df,
      aes(x = .data[[group_var]], y = Inf, label = pval, col = Significance),
      vjust = 2,
      fontface = "bold"
    ) +
    ...
    geom_text(
      data = stats_df,
      aes(x = .data[[group_var]], y = Inf, label = mean),
      vjust = 4,
      fontface = "bold"
    ) +
  ...
  if (target == "births") {
    vp <- vp +
      labs(
        title = ifelse(
          estimand == "diff",
          "Post. Pred. Difference in birth rate",
          "Posterior Predictive Mult. change in birth rate"
        ),
        subtitle = "Per 1,000 women per year"
      )
    ...
    vp <- vp +
      labs(
        title = ifelse(
          estimand == "diff",
          "Post. Pred. Difference in mortality rate",
...
make_violins <- function(
  ...
  if (method == "pred") {
    if (target == "births") {
      ## Compute ratio of birth rates (rates per 1000 people per year (or the rate_normalizer))
      state_df <- merged_df %>%
        group_by_at(c(".draw", ".chain", group_var, "date")) %>%
        mutate(
          outcome = sum(.data[[target]]),
          ypred = sum(ypred),
          denom = mean(.data[[denom]])
        ) %>%
        mutate(
...
make_interval_plot <- function(
  ...
  if (method == "pred") {
    ## Compute difference in death rate per 1000 births (or the rate_normalizer)
    state_df <- merged_df %>%
      group_by_at(c(group_var, ".draw", ".chain", "date")) %>%
      mutate(
        outcome = sum(.data[[target]]),
        ypred = sum(ypred),
        denom = sum(.data[[denom]])
    ...
    ban_states_df <- state_df %>%
      group_by_at(c(".draw", ".chain", setdiff(group_var, "state"), "date")) %>%
      summarize(
        treated = sum(treated),
        untreated = sum(untreated),
        denom = sum(denom)
  ...
  state_df <- state_df %>%
    ...
    mutate(
      state = fct_relevel(state, "Ban States (excl. Texas)", "Ban States")
    ) %>%
    mutate(
      state = fct_recode(
        state,
...
make_violin_diffs <- function(
  ...
  for (i in 1:length(diff_cat1)) {
    state_wide_df <- state_wide_df %>%
      mutate(
        !!paste0(diff_cat1[i], " - ", diff_cat2[i]) := !!sym(diff_cat1[i]) -
          !!sym(diff_cat2[i])
      )
  }

  state_long_df <- state_wide_df %>%
    pivot_longer(
      cols = contains(" - "),
      names_to = "diff_cat",
      values_to = "diff"
  ...
  state_long_df %>%
    ...
    geom_text(
      data = stats_df,
      aes(x = diff_cat, y = Inf, label = pval, col = Significance),
      vjust = 2,
      fontface = "bold"
    ) +
    scale_colour_manual(values = c("red" = "red", "black" = "black")) +
    ...
    geom_text(
      data = stats_df,
      aes(x = diff_cat, y = Inf, label = mean),
      vjust = 4,
      fontface = 'bold'
    ) +
    theme_bw(base_size = 16) +
  ...
  if (target == "births") {
    table_out <- table_out %>%
      cols_label(
        category = "Category",
        outcome = "Births",
        outcome_diff_lower = "Lower",
        outcome_diff_upper = "Upper",
        outcome_diff_mean = "Birth Diff.",
        outcome_rate = "Birth Rate",
        mult_change_lower = "Lower",
        mult_change_upper = "Upper",
        rate_diff = "Rate Diff.",
    ...
    table_out <- table_out %>%
      cols_label(
        ...
        outcome_diff_lower = "Lower",
        outcome_diff_upper = "Upper",
        outcome_diff = "Death Diff.",
        outcome_rate = "Death Rate",
        mult_change_lower = "Lower",
        mult_change_upper = "Upper",
        rate_diff = "Rate Diff.",
        mult_change = "Rate Mult.",
...
make_all_states_plot <- function(dat, focus_state = "Texas") {
  focus_dat <- dat %>%
    ...
    mutate(
      Ef = Ef_samp,
      diff = births - Ef_samp,
      Ef_lower = Ef_samp_lower,
      Ef_upper = Ef_samp_upper
    )
  ...
  gp <- dat %>%
    ...
    group_by(state) %>%
    mutate(mean_births = mean(births), mean_population = mean(population)) %>%
    ungroup() %>%
    ggplot(aes(x = year_mth, y = births)) +
    geom_line(
      aes(y = diff / sqrt(population / 1000), group = state),
      color = "dark gray",
      alpha = 0.5
    ) +
...
make_abs_res_ppc_plot <- function(
  merged_df,
  outcome = "births",
  categories = NULL
) {
...
make_sign_change_ppc_plot <- function(
  merged_df,
  outcome = "births",
  categories = NULL
) {
...
make_unit_corr_ppc_plot <- function(
  merged_df,
  ...
  eval_stats <- merged_df %>%
    filter(date < max_treat_date) %>%
    filter(.draw < ndraws_to_use) %>%
    filter(category %in% categories) %>%
    mutate(
      obs_residual = .data[[outcome]] - exp(mu),
      pred_residual = ypred - exp(mu)
    ) %>%
    group_by(state, category) %>%
    ...
    group_by(category, .draw) %>%
    summarise(
      obs_sval = sqrt(eigen(cor(
        matrix(obs_residual, ncol = length(unique(D))),
        use = "pairwise.complete.obs"
      ))$values[1]),
      pred_sval = sqrt(eigen(cor(
        matrix(
          as.logical(obs_residual) * pred_residual,
          ncol = length(unique(D))
        ),
        use = "pairwise.complete.obs"
      ))$values[1])
    ) %>%
...
get_factor_arrays <- function(
  draws_mat,
  category = "all",
  subgroup_factors = TRUE
) {
  category_code <- 1
  state <- 1
...
## Texas is 44
make_latent_factors_plot <- function(
  factor_list,
  focus_state = "Texas",
  ...
  fact <- exp(as.numeric(
    matrix(
      fl_post_mean[, focus_state_index],
      nrow = nrow(lf_post_mean),
      ncol = NL,
      byrow = TRUE
    ) *
      lf_post_mean
  ))
  fact_lower <- exp(as.numeric(
    matrix(
      fl_post_mean[, focus_state_index],
      nrow = nrow(lf_post_mean),
      ncol = NL,
  ...
  tibble(
    date = rep(1:nrow(lf_post_mean), NL),
    factor = fact,
    ...
    ggplot() +
    geom_ribbon(
      aes(x = date, ymax = factor_upper, ymin = factor_lower),
      fill = "gray",
      alpha = 0.5
    ) +
    facet_wrap(~factor_id) +
    theme_bw() +
    geom_line(aes(x = date, y = factor)) +
    facet_wrap(~factor_id) +
    theme_bw() +
...
make_factor_loadings_plot <- function(
  factor_list,
  state_names,
  factor1 = 1,
  factor2 = 2
  ...
  ggplot(tibble(x = fl[factor1, ], y = fl[factor2, ], label = state_names)) +
